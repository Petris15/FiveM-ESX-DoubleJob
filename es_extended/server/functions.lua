...
ESX.SavePlayer = function(xPlayer, cb)
	...
	-- Job, mafia, loadout and position
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET job = @job, job_grade = @job_grade, mafiajob = @mafiajob, mafiajob_grade = @mafiajob_grade, loadout = @loadout, position = @position WHERE identifier = @identifier', {
			['@job']        = xPlayer.job.name,
			['@job_grade']  = xPlayer.job.grade,
			['@mafiajob']        = xPlayer.mafiajob.name,
			['@mafiajob_grade']  = xPlayer.mafiajob.grade,
...
ESX.DoesMafiaJobExist = function(mafiajob, mafiagrade)
	mafiagrade = tostring(mafiagrade)

	if mafiajob and mafiagrade then
		if ESX.MafiaJobs[mafiajob] and ESX.MafiaJobs[mafiajob].mafiagrades[mafiagrade] then
			return true
		end
	end
