...
TriggerEvent('es:addGroupCommand', 'setmafiajob', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		local xPlayer = ESX.GetPlayerFromId(args[1])

		if xPlayer then
			if ESX.DoesMafiaJobExist(args[2], args[3]) then
				xPlayer.setMafiaJob(args[2], args[3])
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'That job does not exist.' } })
			end
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('setjob'), params = {{name = "id", help = _U('id_param')}, {name = "mafiajob", help = _U('setjob_param2')}, {name = "mafiagrade_id", help = _U('setjob_param3')}}})
