-- Inventory Transfer Logs Made By Petris For Blaze Z Community
givelogswebhook = 'putyourwebhookhere'
...
AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		-- Get job, mafia and loadout
		table.insert(tasks, function(cb)

			local tasks2 = {}

			-- Get job, mafia name, grade and last position
			table.insert(tasks2, function(cb2)

				MySQL.Async.fetchAll('SELECT job, job_grade, mafiajob, mafiajob_grade, loadout, position FROM users WHERE identifier = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
					local job, grade = result[1].job, tostring(result[1].job_grade)
					local mafiajob, mafiagrade = result[1].mafiajob, tostring(result[1].mafiajob_grade)

					if ESX.DoesJobExist(job, grade) and ESX.DoesMafiaJobExist(mafiajob, mafiagrade) then
						local jobObject, gradeObject = ESX.Jobs[job], ESX.Jobs[job].grades[grade]
						local mafiajobObject, mafiagradeObject = ESX.MafiaJobs[mafiajob], ESX.MafiaJobs[mafiajob].mafiagrades[mafiagrade]
						...
						userData.mafiajob.id    = mafiajobObject.id
						userData.mafiajob.name  = mafiajobObject.name
						userData.mafiajob.label = mafiajobObject.label

						userData.mafiajob.grade        = tonumber(mafiagrade)
						userData.mafiajob.grade_name   = mafiagradeObject.name
						userData.mafiajob.grade_label  = mafiagradeObject.label
						userData.mafiajob.grade_salary = mafiagradeObject.salary
					else
						...
						local mafiajob, mafiagrade = 'nomafia', '0'
						local mafiajobObject, mafiagradeObject = ESX.MafiaJobs[mafiajob], ESX.MafiaJobs[mafiajob].mafiagrades[mafiagrade]
						...
						userData.mafiajob.skin_male    = {}
						userData.mafiajob.skin_female  = {}
					end
		...
		Async.parallel(tasks, function(results)
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.mafiajob, userData.loadout, userData.playerName, userData.lastPosition)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				TriggerClientEvent('es_extended:giveanimation')
				-- LOGS
				PerformHttpRequest(givelogswebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Give Item Logs", description = "**Item:** "..itemName.." x "..itemCount.."\n**Sender Name:** "..GetPlayerName(_source).."\n**Sender ID:** ".._source.."\n**Sender Identifier:** ".. GetPlayerIdentifier(_source) .."\n**Reciever Name:** "..GetPlayerName(target).."\n**Player ID:** "..target.."\n**Player Identifier:** ".. GetPlayerIdentifier(target) .."", color=1146986}}}),  { ['Content-Type'] = 'application/json' })
				-- LOGS
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			-- LOGS
			PerformHttpRequest(givelogswebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Give Money Logs", description = "**Money:** "..itemCount.."€\n**Sender Name:** "..GetPlayerName(_source).."\n**Sender ID:** ".._source.."\n**Sender Identifier:** ".. GetPlayerIdentifier(_source) .."\n**Reciever Name:** "..GetPlayerName(target).."\n**Player ID:** "..target.."\n**Player Identifier:** ".. GetPlayerIdentifier(target) .."", color=3066993}}}),  { ['Content-Type'] = 'application/json' })
			-- LOGS
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			-- LOGS
			PerformHttpRequest(givelogswebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Give Black Money Logs", description = "**Black Money:** "..itemCount.."€\n**Sender Name:** "..GetPlayerName(_source).."\n**Sender ID:** ".._source.."\n**Sender Identifier:** ".. GetPlayerIdentifier(_source) .."\n**Reciever Name:** "..GetPlayerName(target).."\n**Player ID:** "..target.."\n**Player Identifier:** ".. GetPlayerIdentifier(target) .."", color=3087}}}),  { ['Content-Type'] = 'application/json' })
			-- LOGS
	...
	elseif type == 'item_weapon' then
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				-- LOGS
				PerformHttpRequest(givelogswebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Give Weapon Logs", description = "**Weapon:** "..weaponLabel.." ["..itemCount.."]\n**Sender Name:** "..GetPlayerName(_source).."\n**Sender ID:** ".._source.."\n**Sender Identifier:** ".. GetPlayerIdentifier(_source) .."\n**Reciever Name:** "..GetPlayerName(target).."\n**Player ID:** "..target.."\n**Player Identifier:** ".. GetPlayerIdentifier(target) .."", color=15158332}}}),  { ['Content-Type'] = 'application/json' })
				-- LOGS
				...
				-- LOGS
				PerformHttpRequest(givelogswebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Give Weapon Logs", description = "**Weapon:** "..weaponLabel.."\n**Sender Name:** "..GetPlayerName(_source).."\n**Sender ID:** ".._source.."\n**Sender Identifier:** ".. GetPlayerIdentifier(_source) .."\n**Reciever Name:** "..GetPlayerName(target).."\n**Player ID:** "..target.."\n**Player Identifier:** ".. GetPlayerIdentifier(target) .."", color=15158332}}}),  { ['Content-Type'] = 'application/json' })
				-- LOGS
