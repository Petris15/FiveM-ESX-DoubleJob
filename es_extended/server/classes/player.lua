function CreateExtendedPlayer(player, accounts, inventory, job, mafiajob, loadout, name, lastPosition)
	local self = {}
	...
	self.getMafiaJob = function()
		return self.mafiajob
	end
	...
	self.setMafiaJob = function(mafiajob, mafiagrade)
		mafiagrade = tostring(mafiagrade)
		local lastMafiaJob = json.decode(json.encode(self.mafiajob))

		if ESX.DoesMafiaJobExist(mafiajob, mafiagrade) then
			local mafiajobObject, mafiagradeObject = ESX.MafiaJobs[mafiajob], ESX.MafiaJobs[mafiajob].mafiagrades[mafiagrade]

			self.mafiajob.id    = mafiajobObject.id
			self.mafiajob.name  = mafiajobObject.name
			self.mafiajob.label = mafiajobObject.label

			self.mafiajob.grade        = tonumber(mafiagrade)
			self.mafiajob.grade_name   = mafiagradeObject.name
			self.mafiajob.grade_label  = mafiagradeObject.label
			self.mafiajob.grade_salary = mafiagradeObject.salary


			TriggerEvent('esx:setMafiaJob', self.source, self.mafiajob, lastMafiaJob)
			TriggerClientEvent('esx:setMafiaJob', self.source, self.mafiajob)
		else
			print(('es_extended: ignoring setMafiaJob for %s due to mafiajob not found!'):format(self.source))
		end
	end
