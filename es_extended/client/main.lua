...
AddEventHandler('esx:addInventoryItem', function(item, count)
	...
	if ESX.UI.Menu.IsOpen('default', 'petris-esxbase', 'inventory') then
		ESX.ShowInventory()
	end
end)
...
AddEventHandler('esx:setMafiaJob', function(mafiajob)
	if Config.EnableHud then
		ESX.UI.HUD.UpdateElement('mafiajob', {
			mafiajob_label   = mafiajob.label,
			mafiagrade_label = mafiajob.grade_label
		})
	end
end)
...
RegisterNetEvent('es_extended:giveanimation')
AddEventHandler('es_extended:giveanimation', function()
	playAnim('mp_common', 'givetake1_a', 500)
end)
